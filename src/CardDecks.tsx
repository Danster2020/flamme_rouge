...
export default function CardDecks({ G, onRslotClick, onSslotClick, playerID }) {
    ...
    const [showRecDeckDialogR, setShowRecDeckDialogR] = useState(false); // Step 1

    function getRecycleDeckR() {
        let arr = []
    ...
    const handleRecDeckDialogToggleR = () => {
        setShowRecDeckDialogR(showRecDeckDialogR => !showRecDeckDialogR); // Step 2
    };

    return (
        <>
            <RecDeckDialog boolean={showRecDeckDialogR} cardsArray={getRecycleDeckR()}></RecDeckDialog>
            ...
            <div className="fixed bottom-0 left-0 z-20">
                <div className="relative flex text-3xl">
                    ...
                    <div className='flex flex-col'>
                        <FontAwesomeIcon onClick={handleRecDeckDialogToggleR} className='text-white' icon={icon({ name: 'clock-rotate-left' })} />
